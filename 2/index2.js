...
function main() {
  ...
  // premultipliedAlpha为false 时，alpha 是根据canvas背景色进行计算。
...
function drawScene(gl, programInfo, buffers) {
  ...
  {
    // const numComponents = 2;// 这里的2代表每个坐标只需要用两个数值表示（X,Y）
    const numComponents = 3;
    const type = gl.FLOAT;
    const normalize = false;
    const stride = 0;
    const offset = 0;
    //buffers.position 是由gl.createBuffer()生成的对象
...

  {
    ...
    //gl.TRIANGLE_STRIP 代表下一个三角形，会复用上两点作为顶点。 所以四个点能画出两个三角形
